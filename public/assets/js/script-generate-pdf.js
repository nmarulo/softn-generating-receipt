...
function createPDF(client, products, receipt, options, dataUrlString) {
	...
	if (receipt['receipt_type'].toUpperCase() == 'PRESUPUESTO') {
		receiptNumber = '';
		strNumber = '';
	} else {
	...
	doc.text(marginX + 20, marginY + 10, receiptNumber);
	
	doc.setTextColor(102, 102, 102);
	doc.text(marginX, marginY + 16, 'Fecha:');
	doc.setTextColor(0, 0, 0);
	...
	if(receipt['receipt_license_plate'] !== null){
		...
		doc.setTextColor(0, 0, 0);
		doc.text(marginX + 20, marginY + 22.5, receipt['receipt_license_plate']);
	}
